...
//!       needle into .gist/ plus the EXACT indexed file list. The
...
test {
    // The engine tests ride the engine package, and `stats.zig`'s bootstrap-CI +
    // Mann-Whitney tests ride it too now that the instruments live there — so
...
// package rather than beside the engine it times: the session lane drives a
// REAL `gist serve` daemon over a REAL socket, and the engine package is
// upstream of the product — it cannot reach down here.
const product = @import("gist");
const serve = product.session.serve; // the resident daemon
const proto = product.session.protocol; // the UDS wire codec the client speaks
