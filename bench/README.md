...
| [`apparatus/`](apparatus/README.md)       | Not a claim: the corpus the races run over (`corpora/`) and the sibling resolver every gate sources (`roots.sh`).                                                                                                                                                                                                                                 |
...
multipattern, sliver), `bounds/` (distance from a stated limit — the µarch port
bound, the memory roofline, the information-theoretic candidate-byte floor), and
...
competitor race here and an engine rung there map 1:1 by class name).
