...
# PRODUCT, not KERNEL: this gate oracles the `gist` binary, which is built by
# the sibling product checkout. Counting `../` up to the package root finds the
# engine, whose zig-out holds no `gist` — and the resulting "not a byte-identical
# drop-in" verdict would be about a binary that was never run.
