...
if [[ "${allow_known}" -eq 1 ]]; then
  run "rgsuite parity (check_results.py --allow-fail)" \
    python3 bench/conformance/rgsuite/check_results.py --allow-fail
else
  run "rgsuite parity (check_results.py)" python3 bench/conformance/rgsuite/check_results.py
fi
...
# can never reach the perf phase. See `bench/conformance/rgsuite/modes.py`.
run "multiline parity -U (modes.py)" \
  python3 bench/conformance/rgsuite/modes.py run --mode multiline
run "pcre parity -P (modes.py)" python3 bench/conformance/rgsuite/modes.py run --mode pcre
...
# phase — this gate caught exactly that. See
# `bench/conformance/gates/oracle/indexed_pcre_oracle.py`.
run "indexed-PCRE2 oracle (indexed_pcre_oracle.py)" \
  python3 bench/conformance/gates/oracle/indexed_pcre_oracle.py run
...
# reach the perf phase. See `bench/conformance/rgsuite/flags.py`.
run "flags parity (flags.py, both engines)" \
  python3 bench/conformance/rgsuite/flags.py run --engine both
...
# The three surfaces a MINED rg suite structurally cannot reach: the by-value escape
# family (rg *rejects* `\N{NAME}` and every octal spelling, so its own suite has no
# case for them), the `--null-data` record model (rg's suite holds no record with an
# interior newline, so it never asks what `^` means inside one), and verbose mode
# crossed with the two places verbose is not supposed to reach (rg's own `(?:…)`
# pattern wrapper loses its `)` to a trailing `#` comment, and rg strips trivia
# through a `[…]` where `re` and PCRE2 stop). Ripgrep is the oracle for every cell it
# can answer; where the two disagree Python `re` referees over haystacks split by
# hand, and each of the six declared boundaries re-proves its own mechanism per run.
# Every family's population is pinned by EXACT count in `records_baseline.json`, so a
# boundary that grows or shrinks fails here. Blocking so an escape, record-model, or
# verbose regression can't reach the perf phase. See
# `bench/conformance/rgsuite/records.py`.
run "escape + record-model + verbose parity (records.py)" \
...
# phase. See `bench/conformance/shapes/`.
run "CLI-shape matrix parity (shapes.py)" python3 bench/conformance/shapes/shapes.py parity
run "line-output parity (line_parity.sh)" bash bench/conformance/gates/parity/line_parity.sh
run "Unicode parity (unicode_parity.sh)" bash bench/conformance/gates/parity/unicode_parity.sh
run "index-elision parity (index_elision_parity.sh)" \
  bash bench/conformance/gates/parity/index_elision_parity.sh
run "enumeration determinism (enum_determinism.sh)" \
  bash bench/conformance/gates/contract/enum_determinism.sh
run "fail-closed contract (fail_closed.sh)" bash bench/conformance/gates/contract/fail_closed.sh
run "freshness (freshness_fs.sh)" bash bench/conformance/gates/contract/freshness_fs.sh
...
# What the certificate claimed over time is in bench/certificate/ledger/LEDGER.md.
#
# The guard reports an absent bundle as exit 2, and a missing INTERPRETER SCRIPT
# is also exit 2 — so while these paths were stale (pre-restructure `bench/certify/`)
# this gate read its own broken invocation as "no certificate published yet",
# printed a NOTE, skipped the ratio floors, and passed. Infrastructure that cannot
# run is not evidence of anything, so the script is asserted before it is trusted.
cert_guard=bench/certificate/guard/artifacts.py
if [[ ! -f "${cert_guard}" ]]; then
  echo "FAIL: certificate guard missing at ${cert_guard} — cannot judge the committed bundle"
  exit 1
fi
set +e
python3 "${cert_guard}" \
...
case "${art_rc}" in
  0)
    ...
    run "cold speedup floors (guard/ratio.py --committed)" \
...
run "warm session floors (gate_session.py --committed)" \
  python3 bench/dominance/session/gate_session.py --committed
...
run "CLI-shape matrix floors (shapes.py gate)" \
  python3 bench/conformance/shapes/shapes.py gate
...
# `bench/conformance/rgsuite/transforms.py` `do_bench`.
run "-z decode speed floor (transforms.py bench)" python3 bench/conformance/rgsuite/transforms.py bench
run "macro certificate (mint.sh)" bash bench/certificate/mint/mint.sh
run "index-size accounting (index_size_accounting.py)" \
  python3 bench/conformance/gates/oracle/index_size_accounting.py
# No `--require-head` here: the restructure deliberately dropped it, because a
# bundle's integrity is judged from the committed bytes rather than from whatever
# HEAD happens to be (`guard/release.py` states the same rule). The flag outlived
# the semantic by long enough to become an argparse error — which this gate then
# read as exit 2, "no certificate yet".
run "fresh certificate artifacts (guard/artifacts.py)" \
  python3 bench/certificate/guard/artifacts.py --artifacts
