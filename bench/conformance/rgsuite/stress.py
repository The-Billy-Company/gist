...
CASES: tuple[tuple[str, str, list[str]], ...] = (
    ...
    (
        "pattern",
        "lazy-multiline-count",
        ["-U", "-c", "-e", r"a[\s\S]*?a", "stress/one_line_64m.txt"],
    ),
    (
        "pattern",
        "greedy-multiline-scan",
        ["-U", "-c", "-e", r"a[\s\S]*a", "stress/one_line_64m.txt"],
    ),
...
def main() -> int:
    ...
    ap.add_argument(
        "--rss-cap-mb", type=float, default=1024.0, help="peak-RSS bound per case (MiB)"
    )
    ...
    if not Path(GIST).exists():
        print(
            f"stress: no gist binary at {GIST} (zig build -Doptimize=ReleaseFast)", file=sys.stderr
        )
        return 2
    ...
    try:
        ...
        for axis, name, tail in cases:
            ...
            records.append(
                {"axis": axis, "case": name, "argv": tail, "gist": g.as_dict(), "rg": r.as_dict()}
            )
            print(
                f"{'FAIL' if g.verdict != 'clean' else 'ok  '} {axis:8} {name:26} "
                f"gist={g.cell():22} rg={r.cell()}",
                flush=True,
            )
        ...
        if rg_worse:
            print(
                f"ripgrep is NOT clean on {len(rg_worse)}: "
