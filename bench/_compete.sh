...
HAVE_RG=0;      have rg          && HAVE_RG=1
HAVE_UGREP=0;   have ugrep       && HAVE_UGREP=1
HAVE_AG=0;      have ag          && HAVE_AG=1
HAVE_GGREP=0;   have ggrep       && HAVE_GGREP=1
HAVE_GITGREP=0; have git         && HAVE_GITGREP=1
HAVE_CSEARCH=0; have csearch && have cindex && HAVE_CSEARCH=1
HAVE_ZOEKT=0;   have zoekt && have zoekt-index && HAVE_ZOEKT=1
...
compete_build_csearch() {
    ...
    [[ -f "${PATHS_LIST}" ]] || { echo "  csearch: no ${PATHS_LIST} (run gist index first)"; HAVE_CSEARCH=0; return 0; }
    mkdir -p "${COMPETE_DIR}"; rm -f "${CSEARCH_IDX}"
...
compete_build_zoekt() {
    ...
    mkdir -p "${COMPETE_DIR}"; rm -rf "${ZOEKT_DIR}"; mkdir -p "${ZOEKT_DIR}"
    ...
    t1="$(python3 -c 'import time;print(time.time())')"
    secs="$(python3 -c "print('%.1f'%(${t1}-${t0}))")"
    du_out="$(du -sk "${ZOEKT_DIR}")"; kb="${du_out%%[!0-9]*}"   # leading kb field, no pipe
...
_xdir_flags() { local f=""; for d in "${XDIRS[@]}"; do f+=" --exclude-dir=${d}"; done; echo "${f}"; }
...
need_hyperfine() { have hyperfine || { echo "need hyperfine (brew install hyperfine)"; exit 1; }; }
...
hf_mean() {
    local warmup="$1" runs="$2" cmd="$3" js; js="$(mktemp)"
...
ratio() { [[ "$1" = "?" || "$2" = "?" ]] && { echo "?"; return; }; python3 -c "print('%.1fx'%(${1}/${2}))" 2>/dev/null || echo "?"; }
