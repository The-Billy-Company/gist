...
fn writeArtifacts(gpa: std.mem.Allocator, io: std.Io, corpus: *const corpus_mod.Corpus, meter: *pmu.Meter, rows: []const Row, mib: f64, brand: []const u8, qos: []const u8) !void {
    ...
    try md.appendSlice(gpa, "A *dominance-and-fit certificate* is built in five layers, cheapest evidence first. It certifies measured dominance over a named baseline and each layer's fit against a stated bound — never universal or hardware optimality:\n\n");
