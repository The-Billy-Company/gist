...
Where Layer A proves gist is *empirically fastest in its class* and Layer B
that its hot loop matches the static instruction-level bound, Layer C proves
the last hardware claim: gist's cycles/byte sit on *this machine's* memory
...
*slows* a trial, never inflates it, so the max is the cleanest ceiling
...
Frequency (only needed for the *derived* cycles/byte ceiling) is measured via
...
- **John D. McCalpin, "Memory Bandwidth and Machine Balance in Current High
  Performance Computers" (1995), *IEEE TCCA Newsletter*.** The STREAM
...
- **Samuel Williams, Andrew Waterman, David Patterson, "Roofline: An
  ...
  *Communications of the ACM* 52(4):65-76.** The roofline model itself — a
