...
//!
//! hydra's verb surface is closed and static (five verbs over the hydra
...
const manifest =
    ...
    \\    },
    \\    "quote": {
    \\      "summary": "rewrite <text> as maximal verbatim quotations from the WHOLE corpus, priced in bits (Ziv-Merhav cross-parse on the persisted codex shelf); bits/byte = corpus-conditional compression rate, low = the corpus already knows it; each phrase attributed to one exemplar file; requires `gist codex build`",
...
test "hydra --schema is valid JSON naming all five verbs" {
    const t = std.testing;
    ...
    try t.expect(verbs.contains("quote"));
