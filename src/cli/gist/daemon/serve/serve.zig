...
/// generation the READY frame reports (monotonic across accepts).
const Client = struct { stream: net.Stream, gen: u64 };
...
pub fn run(gpa: std.mem.Allocator, io: std.Io, roots: []const []const u8, socket_path: []const u8) !void {
    ...
    serve_loop: while (true) {
        ...
        for (clients.items, pfds.items[1..]) |*c, pfd| {
            ...
            const after = serveFrame(&session, gpa, c.stream.socket.handle, c.gen);
...
fn serveFrame(session: *ResidentSession, gpa: std.mem.Allocator, fd: std.posix.fd_t, session_gen: u64) After {
    var frame = protocol.recvFrame(gpa, fd) catch return .drop; // closed/oversized/bad → drop peer
    defer frame.deinit();
    switch (frame.op) {
        .hello, .status => sendReady(session, gpa, fd, session_gen) catch return .drop,
        .ping => protocol.sendFrame(gpa, fd, .pong, "") catch return .drop,
        .query => handleQuery(session, gpa, fd, frame.payload()) catch return .drop,
