...
doc_radar:
  ...
  sentinels:
    ...
    - file: pkg/kernels/irregex/src/surface/cli/beacon.zig
      description: the hyperlink layer keeps its five decisions separate — read a value, judge the terminal, pick a destination, refuse an anchor a control byte would tear, split a per-file URL
...
| `beacon.zig`   | Clickable results (OSC-8): reads one value that may name a posture, a destination, or both (`wish`); judges whether the terminal will render a link rather than show its bytes (`speaks`); infers the editor from the environment when nobody said (`destination`); declines a frame whose anchor a control byte in the filename would tear across two lines (`tears`); and splits each file's URL once into a `Waypoint` so a row costs a memcpy and a decimal   |
