...
| `emit.zig`          | Result rows: `jsonStr` (the one NDJSON escaper, re-exported from `jsonstr.zig` so every face escapes identically), `jsonRow` (one object from a comptime field spec), `emitRow` (text vs `--json` off one bool), and `anchor`/`locator` — wrap a path, or a path and its line, in whatever click target the run resolved                                                                                                                                                                                    |
| `jsonstr.zig`       | The one JSON string escaper every face that writes JSON/NDJSON shares (`write`): serde_json's / ripgrep's exact discipline — `"` and `\` backslash-escaped, the five C0 short forms, every other control as `\u00XX`, valid UTF-8 raw — over a vectorized escape scan, so each escape-free span costs one comparison and one memcpy rather than a byte-at-a-time append. `rg --json`, the relate/irregex verb rows, and the `--schema` manifest carried three near-copies that had quietly diverged         |
