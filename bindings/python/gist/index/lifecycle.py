"""Typed lifecycle and capability introspection — gist's trigram index.
...
*sees* that tier: the trigram index behind exact search (`status`/`index`) and
the binary's capability manifest (`capabilities`).

Relate's atlas / fragment / shelf lifecycle lives in the `relate` package.
"""
