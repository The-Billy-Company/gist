"""Agent-facing request mapping.

An assistant operating a user's machine and a coding agent working a repository are the
...
. Local callers hand the mapped request straight to `gist.run`; a
...
from irgx.contract import REQUEST_OPTIONS
from irgx.request import SearchRequest

from ._contract import ALIASES, ROUTING_KEYS
...
def request_from_tool(payload: Mapping[str, object]) -> SearchRequest:
    ...
    An agent `fs_search(place, query, glob, context_lines, semantic, at)` and a
