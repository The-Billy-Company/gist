...
`dlopen`s `libgist.{dylib,so}` in ABI mode (no C compiler, no dev headers, no
per-Python build) and drives the `gist_open`/`gist_search`/`gist_close` C ABI
(`../../../include/gist.h`, implemented in `src/ffi/session.zig`). It holds one
...
can't open, or the engine returns `GIST_STALE` (an unsupported pattern) — so the
...
# C declarations mirroring include/gist.h (the session ABI subset). ABI mode
...
_GIST_OK = 0  # ran, no match; a negative status (e.g. GIST_STALE=-1) → cold.


def _dylib_name() -> str:
    if sys.platform == "darwin":
        return "libgist.dylib"
...
def _resolve_lib() -> str | None:
    """`$GIST_LIB` override, else the kernel's built `zig-out/lib/libgist.<ext>`.
...
class Handle:
    ...
    def __init__(self, ffi: FFI, lib: object) -> None:
        ...
        rc = lib.gist_open(ffi.NULL, 0, out)
    ...
    def search(self, request: SearchRequest) -> list[Match] | None:
        ...
        Returns None to fall back cold (closed handle, or a `GIST_STALE`/error
