...
This module is the evidence, run over doc_radar's *real* query corpus (never a
...
- **GIST ≈1.6x faster than `rg`** across the batch — but `gist_cold ≈
  gist_warm`. doc_radar's `still_here` pins search *single explicit files*, so
  the persisted index adds mmap cost without pruning benefit; the win here is
  raw per-invocation speed, not the index.
- The material *index/session* warm win the plan hypothesized is for
